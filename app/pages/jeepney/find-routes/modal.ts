...
export class MyModal {
  ...
  constructor(private navCtrl: NavController, private viewCtrl: ViewController, private modalCtrl: ModalController, private navParams: NavParams, private dataService: DataService, private translate: TranslateService){
    this.points = [];
      ...
      if(data.res.rows.length > 0) {
        for(var i = 0; i < data.res.rows.length; i++) {
          if (navigator.language.split('-')[0]=='en') {
            ...
            translate.get(data.res.rows.item(i).text).subscribe((res: string) => {
